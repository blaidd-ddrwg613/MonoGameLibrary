...
public class Camera2D : IDisposable
{
    ...
    public void CenterOnTarget(Rectangle rec)
    {
        ...
        var fat1 = ResolutionRenderer.VirtualWidth / (float)ResolutionRenderer.VirtualHeight;
        var fat2 = rec.Width / (float)rec.Height;
        float ratio;
        if (fat2 >= fat1) ratio = ResolutionRenderer.VirtualWidth / (float)rec.Width;
    ...
    public Matrix GetViewTransformationMatrix()
    {
        if (_isViewTransformationDirty)
        {
            ...
            _resTranslationVector.X = ResolutionRenderer.VirtualWidth * 0.5f;
            _resTranslationVector.Y = ResolutionRenderer.VirtualHeight * 0.5f;
